...
void send_updates(udp_layer_t* udp, ripv2_route_table_t* table, int is_triggered)
{
    ...
    for (int i = 0; i < table_size; i++)
    {
        ...
        // Simplified version where every route that is not locally connected is set to 16 when sending periodic updates
                     ...
                     uint16_t src_port)
{
    ...
    if (request_all)
    {
        printf("[RIPv2] Solicitud de tabla completa recibida de %d.%d.%d.%d\n", src_ip[0], src_ip[1], src_ip[2],
               src_ip[3]);
